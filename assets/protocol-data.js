/**
 * DUST-20 — single source of truth.
 *
 * Every field table, formula, example, validation message, conformance vector
 * and machine-readable artifact on this site is generated from this module.
...
export const SOURCES = {
  ...
  inscribe: {
    id: 'inscribe',
    label: 'Bitcoin Universe Inscribe',
    url: 'https://inscribe.bitcoinuniverse.io',
    note: 'Application that builds DUST-20 deploy and mint inscriptions.',
  },
  docs: {
    id: 'docs',
...
export const FIELDS = [
  ...
  },
  {
    ...
    example: 'dust',
    rule:
      'The identity of a ticker is its NFC form case-folded to lower case. A mint resolves against that folded identity.',
    status: 'universe',
    source: ['universe'],
    notes:
  ...
  },
  {
    ...
    type: 'integer string',
    required: true,
    constraint: 'Canonical positive integer string matching /^[1-9][0-9]*$/, at most 2^128 - 1.',
    example: '1000000',
  ...
  },
  {
    ...
    required: true,
    constraint:
      'Canonical positive integer string, at most Bitcoin’s monetary supply of 2,100,000,000,000,000 satoshis.',
    example: '546',
    rule: 'The exact number of satoshis that back one unit for the entire life of the ticker.',
    status: 'verified',
    source: ['legacy', 'universe'],
    notes:
      '546 is a common documented example because it is Bitcoin’s usual dust threshold for P2WPKH. It is not a default. A payload that omits unit_sats is rejected — never assume 546.',
  },
  {
    name: 'max_sats',
    ops: ['deploy'],
    ...
    required: true,
    constraint:
      'Canonical positive integer string that equals supply × unit_sats and does not exceed 2,100,000,000,000,000.',
    example: '546000000',
    rule: 'Rejected unless it equals supply × unit_sats exactly, computed with exact integer arithmetic.',
    status: 'verified',
    source: ['legacy', 'universe'],
    notes:
      'This is a redundancy check, not an independent quantity. It exists so a reader can detect a builder that computed the backing with floating point or the wrong unit size. See the reconciliation note about earlier reports that this field was optional.',
  },
  {
    name: 'lim_sats',
    ops: ['deploy'],
    ...
    required: false,
    constraint:
      'Optional canonical non-negative integer string, at most max_sats. Omitted or "0" means no per-mint limit.',
    example: '54600',
    rule:
      'When present and greater than zero, every mint of this ticker must satisfy sats ≤ lim_sats.',
    status: 'verified',
    source: ['legacy', 'universe'],
    notes:
  ...
  },
  {
    ...
    required: true,
    constraint:
      'Canonical positive integer string that equals amt × unit_sats of the resolved deployment.',
    example: '54600',
    rule:
      'Rejected unless it equals amt × unit_sats, and unless the Bitcoin output carrying the inscription holds exactly this many satoshis.',
    status: 'verified',
    source: ['legacy', 'universe'],
    notes:
...
/* ------------------------------------------------------------------ *
 * Formulas — the only place arithmetic is written down
 * ------------------------------------------------------------------ */
...
export const FORMULAS = [
  ...
  },
  {
    ...
    plain: 'The Bitcoin output that carries the mint inscription must hold exactly the declared satoshis.',
  ...
  },
  {
    id: 'unit-cap',
    op: 'deploy',
    expression: 'mint_unit_cap = ⌊ lim_sats ÷ unit_sats ⌋',
  ...
  },
  {
    ...
    example: '91 units in → 10 to the receiver + 81 as change + 0 burned',
    status: 'universe',
  },
  {
    id: 'backing',
  ...
  },
  {
    id: 'fee',
    op: 'transfer',
    expression: 'fee = Σ inputs − Σ outputs',
    plain:
...
/* ------------------------------------------------------------------ *
 * Rules that are not expressible as a single field constraint
 * ------------------------------------------------------------------ */
...
export const RULES = [
  {
    id: 'payload-shape',
    scope: 'reader',
    title: 'A payload is a flat JSON object of strings',
    body:
      'Inscription content must be valid UTF-8 and a single flat JSON object whose every value is a string. Nested objects, arrays, numbers, booleans and null are rejected. So are duplicate keys, trailing content after the closing brace, and content larger than 4096 bytes.',
    status: 'universe',
    source: ['universe'],
  },
  {
    id: 'exact-keys',
    scope: 'reader',
    title: 'Unknown fields are rejected, not ignored',
    body:
      'Each operation declares an exact key set. A deploy carrying an extra field, or a mint missing one, does not degrade to a warning — the whole payload is rejected. This is stricter than "ignore what you do not understand" and prevents a malformed message from being read two different ways.',
    status: 'universe',
    source: ['universe'],
  },
  {
    id: 'canonical-integers',
    scope: 'reader',
    title: 'Integers are canonical decimal strings',
    body:
      'Every numeric field is a string. Leading zeros, a leading plus, a minus sign, whitespace, decimal points and exponent notation are all rejected. Implementations must use exact integer arithmetic — a 64-bit float silently loses precision above 2^53, well inside the satoshi range.',
    status: 'verified',
    source: ['legacy', 'universe'],
  },
  {
    id: 'first-deploy-wins',
    scope: 'deploy',
    title: 'The first valid deployment of a ticker wins',
    body:
      'Ticker identity is the NFC form of tick, case-folded to lower case. Once a valid deployment exists for an identity, every later deployment of that identity is rejected. "DUST", "dust" and "Dust" are the same ticker.',
    status: 'universe',
    source: ['universe'],
  },
  {
    ...
    body:
      'A mint naming a ticker that has no accepted deployment is rejected. The unit size used to check the mint always comes from the resolved deployment, never from the mint payload.',
    status: 'verified',
    source: ['legacy', 'universe'],
  },
  {
    ...
    body:
      'Previously minted units plus amt must not exceed supply. There is no partial fill: a mint that would cross the supply ceiling is rejected in full rather than trimmed.',
    status: 'verified',
    source: ['legacy', 'universe'],
  },
  {
    id: 'mint-placement',
    scope: 'mint',
    title: 'The inscription must begin at offset zero of its output',
    body:
      'The mint inscription must sit at satoshi offset 0 of the output that carries it, that output must hold exactly sats satoshis, and the output must not already carry a DUST-20 allocation. A mint whose padding does not match its sats field exactly is rejected.',
    status: 'universe',
    source: ['universe'],
  },
  {
    id: 'no-transfer-inscription',
    scope: 'transfer',
    title: 'No transfer inscription is valid',
    body:
      'DUST-20 uses inscriptions only for deploy and mint. Movement happens by spending the satoshis that carry the units. An inscription with op: "transfer" is rejected — it does not move anything, and treating it as a transfer is a compatibility error.',
    status: 'universe',
    source: ['universe'],
  },
  {
    id: 'ordinal-flow',
    scope: 'transfer',
    title: 'Units follow satoshis in first-in-first-out order',
    body:
      'Lay every input value end to end to form one satoshi range, and every output value end to end to form another. A colored allocation occupies a contiguous span in the input range. Each unit is a block of unit_sats satoshis inside that span, and it survives only where a whole block lands inside a single output.',
    status: 'universe',
    source: ['universe'],
  },
  {
    id: 'burn-conditions',
    scope: 'transfer',
    title: 'Three ways units are destroyed',
    body:
      'A unit is burned when its satoshis fall past the last output and become the miner fee, when they land in an output the indexer cannot attribute to a supported address, or when they straddle an output boundary so no whole unit fits. Burns are silent on Bitcoin: the transaction confirms normally.',
    status: 'universe',
    source: ['universe'],
  },
  {
    id: 'partial-send-change',
    scope: 'transfer',
    title: 'A partial send needs an explicit colored change output',
    body:
      'If you spend an allocation of 91 units and send 10, the other 81 need their own output with at least 81 × unit_sats satoshis, aligned so each unit fits whole. Omitting that output does not keep the units — it burns them.',
    status: 'verified',
    source: ['legacy', 'universe'],
  },
  {
    id: 'fee-separation',
    scope: 'transfer',
    title: 'Fund miner fees with ordinary bitcoin',
    body:
      'Colored satoshis and fee funding are different concepts. Add a separate cardinal input to pay the fee. Shaving satoshis off a colored output to cover a fee moves the colored range and can burn the units it was meant to carry.',
    status: 'verified',
    source: ['legacy', 'universe'],
  },
  {
    id: 'merge',
    scope: 'transfer',
    title: 'Merging works when the ticker and unit size match',
    body:
      'Several colored inputs of the same ticker can be combined into one output, provided each contributing unit lands whole inside it. Allocations of different tickers keep separate identities even inside one output; they are tracked by their own offsets.',
    status: 'universe',
    source: ['universe'],
  },
  {
    id: 'reorg',
    scope: 'indexer',
    title: 'Reorganizations roll the ledger back',
    body:
      'On a reorg the indexer emits invalidations newest first, reverses ledger mutations, restores allocations that had been marked spent, removes allocations the orphaned branch created, and replays the replacement branch. A balance is only as final as the depth it was confirmed at.',
    status: 'universe',
    source: ['universe'],
  },
  {
    ...
    body:
      'The production indexer reports coverage as partial: confirmed history and complete holder snapshots are indexed, but there is no exhaustive mempool feed with a stable pending lifecycle. Unconfirmed DUST-20 state is therefore not a reliable input to an irreversible decision.',
    status: 'universe',
    source: ['universe'],
  },
]
...
/* ------------------------------------------------------------------ *
 * Where sources disagree — documented, never silently resolved
 * ------------------------------------------------------------------ */
...
export const RECONCILIATIONS = [
  {
    ...
    legacy:
      'The legacy profile includes max_sats and frames it as supply × unit_sats. Earlier versions of this documentation additionally reported that the Inscribe builder emitted deploys without max_sats, and labelled the question an unreconciled compatibility split.',
    current:
  ...
  {
    id: 'transfer-payload',
    title: 'Is there a canonical transfer payload?',
    legacy:
  ...
  },
  {
    ...
    legacy:
      'The legacy material defines no burn rule, and earlier documentation advised showing recovery actions as potentially allocation-destroying rather than canonical burns.',
    current:
  ...
  {
    id: 'indexer-api',
    title: 'Is there a canonical DUST-20 indexer API?',
    legacy: 'No API schema, pagination model, or response versioning is published in the legacy guide.',
    current:
...
/* ------------------------------------------------------------------ *
 * Open questions — labelled, not invented
 * ------------------------------------------------------------------ */
...
export const OPEN_QUESTIONS = [
  {
    ...
    why:
      'Most of the transfer, burn and ticker-identity behaviour on this site is settled by one production implementation rather than by a multi-party specification.',
    posture:
      'Treat rules marked Current Universe implementation as a working profile. Before relying on them across tools, test against the specific indexer your users will see.',
  },
  {
    id: 'confirmation-threshold',
    question: 'How many confirmations make a DUST-20 balance safe to act on?',
    why:
      'The protocol defines no threshold, and reorg handling restores or removes allocations depending on depth.',
    posture:
  ...
  },
  {
    ...
    why:
      'The production indexer reports partial coverage precisely because it has no exhaustive mempool feed with stable pending and disappearance handling.',
    posture:
  ...
  },
  {
    ...
    why:
      'Replace-by-fee can change the output layout, which changes where colored satoshis land — a replacement can burn units the original preserved.',
    posture:
  ...
  {
    id: 'dust-threshold',
    question: 'What if unit_sats is below Bitcoin’s relay dust threshold?',
    why:
      'DUST-20 permits any positive unit_sats, but Bitcoin nodes will not relay outputs below the standard dust limit for their script type.',
    posture:
      'A deploy with a very small unit_sats can be valid to an indexer yet unspendable in practice. Check the dust threshold for your output type before deploying.',
  },
]
...
export const INDEXER_STAGES = [
  ...
  },
  {
    ...
    body:
      'Apply the flat-string-object reader, exact key sets and canonical integer parsing. Keep parse failures as recorded invalid events rather than dropping them into an unexplained empty result.',
    status: 'universe',
  },
  {
    id: 'resolve',
  ...
  },
  {
    ...
    body:
      'An allocation is an outpoint, a ticker, an amount, a unit size and a satoshi offset inside that output. The offset is not optional bookkeeping — it is what makes the next spend computable.',
    status: 'universe',
  },
  {
    id: 'propagate',
  ...
  },
  {
    ...
    body:
      'Track block association so a reorg can invalidate newest first, reverse mutations, restore spent allocations, remove orphaned ones and replay the replacement branch. Preserve enough history to explain every transition.',
    status: 'universe',
  },
]
...
export const SAFETY = [
  ...
  },
  {
    ...
    body:
      'If you are sending part of an allocation and no colored change output is present, the remainder is not kept — it is burned. This is not a formatting detail. Do not approve it.',
  },
  {
    id: 'fee-inputs',
    severity: 'high',
  ...
  },
  {
    ...
    body:
      'If two indexers describe the same output differently, do not pick the friendlier answer. Show the conflict, block the irreversible step, and keep redacted evidence for investigation.',
  },
  {
    id: 'ambiguity',
    severity: 'high',
    ...
    body:
      'Unexplained satoshi differences, mixed-ticker selections and unattributable outputs are failures. Silently filling in a field, rounding a satoshi value, or assuming an unspecified payload is canonical converts a caught bug into a burned balance.',
  },
  {
    id: 'retry',
    severity: 'medium',
  ...
  },
  {
    ...
    body:
      'A reorg can remove a confirmation and change which transaction actually spent an output. Track confirmation depth and invalidate derived state when the chain moves under you.',
  },
]
